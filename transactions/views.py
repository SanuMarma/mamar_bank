...
from transactions.models import Bankrupt
...
class WithdrawMoneyView(TransactionCreateMixin):
    ...
    def form_valid(self, form):
        amount = form.cleaned_data.get('amount')
        bankrupt = Bankrupt.objects.filter(is_bankrupt=True).first()
        
        if bankrupt and bankrupt.is_bankrupt:
            messages.error(
                self.request,'The bank is bankrupt.'
            )
    ...
    def form_valid(self, form):
        ...
        if amount > sender.balance:
            ...
            if not reciver:
                ...
                send_transaction_email(reciver.user, amount, "Send Money Message", "transactions/sendmoney_email.html")
...
class TransactionReportView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        #jodi user kono type filter na kore taile tar total trasaction report dekhabe
